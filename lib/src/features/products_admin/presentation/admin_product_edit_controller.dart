...
import 'package:demo_app/src/features/products_admin/application/image_upload_service.dart';
...
class AdminProductEditController extends _$AdminProductEditController with NotifierMounted {
  ...
  Future<void> deleteProduct(Product product) async {
    ...
    state = const AsyncLoading();
    final value = await AsyncValue.guard(() => imageUploadService.deleteProduct(product));
    final success = value.hasError == false;
    if (mounted) {
      state = value;
      if (success) {
