...
class ReviewsRepository {
  ...
  static String reviewPath(ProductID id, UserID uid) => 'products/$id/reviews/$uid';
  ...
  DocumentReference<Review?> _reviewRef(ProductID id, UserID uid) =>
      _firestore.doc(reviewPath(id, uid)).withConverter(
            fromFirestore: (doc, _) => doc.exists ? Review.fromMap(doc.data()!) : null,
            toFirestore: (review, _) => review != null ? review.toMap() : {},
          );

  Query<Review> _reviewsRef(ProductID id) =>
      _firestore.collection(reviewsPath(id)).orderBy('date', descending: true).withConverter(
